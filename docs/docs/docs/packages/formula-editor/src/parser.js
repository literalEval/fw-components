...
export class Parser {
    ...
    parseInput(formula, prevCurPos = null, recommendation = null) {
        ...
        tokens.forEach((token) => {
            ...
            formattedString = `${formattedString}<span class="wysiwygInternals ${tokenClassName}">${token}</span>${recommendation ? " " : ""}`;
            // if (isNumber) {
            //   expectation = Expectation.OPERATOR;
            // } else if (isOperator) {
            //   expectation = Expectation.VARIABLE;
    ...
    buildRPN(formula) {
        ...
        for (let token of tokens) {
            if (token == "(") {
                ...
                while (this.mathematicalExpressions.has(operatorStack.top()) &&
    ...
    addParens(formula) {
        ...
        lexedRPN.forEach((symbol) => {
            ...
            if (this.variables.has(symbol) ||
                ...
                if (this.operatorPrecedence[opa] <= this.operatorPrecedence[symbol] ||
        ...
        if (!resultStack.empty()) {
            ...
            throw `${stringRPN} is not a correct RPN`;
    }
    calculate(formula) {
        var _a, _b, _c;
